...
class Location:
    ...
    def __init__(self, size_x, size_y, max_capacity):
        ...
        self.walkers = []
        for i in range(param.statusNum):
            self.walkers.append([])
    ...
    def render(self, virus):
        ...
        if not self.paused:
            ...
            for walkerStatus in range(param.statusNum):
...
def processFairProximity(engine, coord_array):
    for locList in engine.locs:
        for loc in locList:
            ...
            for susceptible in loc.walkers[param.SUSCEPTIBLE]:
                flag = 0    # will hold the period of incubation, if infection happens

                for infected in loc.walkers[param.INFECTED]:
                ...
                if not flag:
                    for asymptomatic in loc.walkers[param.ASYMPTOMATIC]:
                ...
                if flag:
                    ...
                    loc.walkers[param.SUSCEPTIBLE].remove(susceptible)
                    loc.walkers[param.INCUBATION].append(susceptible)
